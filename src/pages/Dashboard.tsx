...
const MetricCard = memo(({ 
  ...
  <div className="bg-card border border-border rounded-lg p-4">
    ...
    <p className="text-xs text-muted-foreground mt-1.5">of {limit}</p>
  </div>
));

MetricCard.displayName = "MetricCard";

// Memoized folder chip
const FolderChip = memo(({ name, onClick }: { name: string; onClick: () => void }) => (
  <button
    onClick={onClick}
    className={cn(
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6 max-w-5xl animate-fade-in">
        {/* Status Strip */}
        <div className="bg-muted/50 rounded-lg px-4 py-3 flex flex-wrap items-center justify-between gap-3">
